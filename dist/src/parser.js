...
function parse(schema, options, keyName, processed = new Map(), usedNames = new Map()) {
...
function parseAsTypeWithCache(schema, type, options, keyName, processed = new Map(), usedNames = new Map()) {
...
function parseNonLiteral(schema, type, options, keyName, processed, usedNames) {
    ...
    switch (type) {
        case 'ALL_OF':
            if (options.allOfExtendsPattern &&
                schema.allOf &&
                schema.allOf.length >= 2 &&
                schema.allOf.slice(2).every(subSchema => !subSchema.properties)) {
                ...
                if (base.type === 'INTERFACE' &&
                    ...
                    return newAllOfExtendsInterface(schema, options, usedNames, base, extension, keyName);
                }
                if (base.type === 'CUSTOM_TYPE' &&
                    base.params &&
                    !base.standaloneName &&
                    !extension.standaloneName &&
                    !schema.properties?.length) {
                    return newAllOfExtendsInterface(schema, options, usedNames, { ...base, standaloneName: base.params, type: 'INTERFACE', superTypes: [], params: [] }, extension, keyName);
                }
            }
            return {
        ...
        case 'ANY':
            return {
                ...(options.unknownAny ? AST_1.T_UNKNOWN : AST_1.T_ANY),
                comment: schema.description,
                deprecated: schema.deprecated,
                keyName,
                standaloneName: standaloneName(schema, keyNameFromDefinition, usedNames, options),
        ...
        case 'UNION':
            return {
                ...
                params: schema.type.map(type => {
                    const member = { ...(0, lodash_1.omit)(schema, '$id', 'description', 'title'), type };
...
function standaloneName(schema, keyNameFromDefinition, usedNames, options) {
    ...
    if (name) {
        return (0, utils_1.generateName)(name, usedNames, schema);
    }
}
...
function newAllOfExtendsInterface(schema, options, usedNames, base, extension, keyName, keyNameFromDefinition) {
    const name = standaloneName(schema, keyNameFromDefinition, usedNames, options);
    return {
        comment: schema.description,
        deprecated: schema.deprecated,
