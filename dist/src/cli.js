...
async function main(argv) {
    ...
    const ISGLOB = argIn && (0, tinyglobby_1.isDynamicPattern)(argIn);
...
async function processGlob(argIn, argOut, argv) {
    const files = await (0, tinyglobby_1.glob)(argIn, { expandDirectories: false }); // execute glob pattern match
    if (files.length === 0) {
...
async function readInput(argIn) {
    if (!argIn) {
        return {
            filename: null,
